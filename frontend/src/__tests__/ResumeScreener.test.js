...
describe("ResumeScreener Component", () => {
  ...
  test("handles backend connection failure", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Backend Status: Disconnected/)
      ).toBeInTheDocument();
    });
  });
  ...
  test("submits form with valid data", async () => {
    ...
    await waitFor(() => {
      expect(ApiService.screenResume).toHaveBeenCalledWith(
        file,
  ...
  test("displays results after successful submission", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Good match for the position")
      ).toBeInTheDocument();
  ...
  test("shows loading state during submission", async () => {
    ...
    ApiService.checkHealth.mockResolvedValue({ status: "healthy" });
    ApiService.screenResume.mockImplementation(
